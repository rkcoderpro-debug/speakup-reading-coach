...
async function handleSession(session){if(session?.user?.id===state.user?.id&&state.profile){state.session=session;return}state.session=session;state.user=session?.user||null;if(!state.user){await multiplayer.stop();hide($("loadingScreen"));hide($("appShell"));hide($("placementOverlay"));show($("authScreen"));return}try{await ensureProfile();renderUser();renderMetrics();hide($("loadingScreen"));hide($("authScreen"));if(!state.profile.onboarding_completed){hide($("appShell"));show($("placementOverlay"));show($("placementIntro"));hide($("placementTest"));hide($("placementFinish"));return}hide($("placementOverlay"));show($("appShell"));await loadPlan();await showPage("today");await multiplayer.start().catch(e=>notice(e.message))}catch(e){console.error(e);hide($("loadingScreen"));show($("authScreen"));status("authError",e.message)}}
...
async function showPage(name){document.querySelectorAll(".page").forEach(hide);document.querySelectorAll(".nav").forEach(b=>b.classList.toggle("active",b.dataset.page===name));show($(`page-${name}`));const titles={today:["TODAY","Daily Reading"],read:["READ","Reading Library"],words:["REVIEW","Difficult Words"],progress:["PROGRESS","Reading Progress"],multiplayer:["TOGETHER","Multiplayer Reading"],profile:["PROFILE","Your Reading Profile"]};$("pageEyebrow").textContent=titles[name][0];$("pageTitle").textContent=titles[name][1];if(name==="today")await loadToday();if(name==="read")await loadLibrary();if(name==="words")await loadWords();if(name==="progress")await loadProgress();if(name==="multiplayer")await multiplayer.page();if(name==="profile"){renderProfilePage();multiplayer.profile()}}
...
async function enterAfterPlacement(){hide($("placementOverlay"));show($("appShell"));renderUser();renderMetrics();renderRoadmap();await showPage("today");await multiplayer.start().catch(e=>notice(e.message))}
...
const multiplayer=createMultiplayer({state,api,esc,notice,openPractice,refreshProfile});
